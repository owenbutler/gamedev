...
cp target/jGameEngine-1.0-SNAPSHOT.jar target/jnlp/
...
# cp ~/lib/lwjgl-1.1.4/jar/jinput.jar target/jnlp/
# cp ~/lib/lwjgl-1.1.4/jar/lwjgl_util.jar target/jnlp/
# cp ~/lib/lwjgl_optional-1.1.4/jar/lwjgl_devil.jar target/jnlp/
# cp ~/lib/lwjgl_optional-1.1.4/jar/lwjgl_fmod3.jar target/jnlp/

# copy jars for native libs
cp ~/lib/lwjgl-1.1.4/native/*.jar target/jnlp/
# cp ~/lib/lwjgl_optional-1.1.4/native/*.jar target/jnlp/


echo "creating jars from gfx and audio"

# create jars from gfx and audio
mkdir -p target/jnlp/gfx/194x
cp -r gfx/194x/* target/jnlp/gfx/194x/
cd target/jnlp/gfx
jar cf ../gfx.jar 194x
cd -
rm -rf target/jnlp/gfx/

mkdir -p target/jnlp/audio/sfx
cp audio/sfx/*.wav target/jnlp/audio/sfx/
cp audio/sfx/*.ogg target/jnlp/audio/sfx/
cd target/jnlp/audio
jar cf ../audio.jar sfx
cd -
rm -rf target/jnlp/audio/


echo "adding manifest to jar"

# add our manifest file to our jar
jar ufm target/jnlp/jGameEngine-1.0-SNAPSHOT.jar manifest.txt
...
cp gfxPSD/194x/splash.gif target/jnlp/
cp gfxPSD/194x/icon.gif target/jnlp/